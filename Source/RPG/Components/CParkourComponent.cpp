...
FParkourData::FParkourData():
	Type(EParkourType::Max), Montage{}, SectionName(""), MinDistance(0.0f), MaxDistance{}, Extent(0.0f), bFixedCamera(false)
{
}
...
void FParkourData::PlayMontage(ACharacter* InCharacter)
{
	if (bFixedCamera)
	{
		UCMovementComponent* movement = Cast<UCMovementComponent>(InCharacter->GetComponentByClass(UCMovementComponent::StaticClass()));
...
DataTable{},DebugType(EDrawDebugTrace::None)
{
...
void UCParkourComponent::BeginPlay()
{
	...
	TArray<USceneComponent*> com{};
	OwnerCharacter->GetComponents<USceneComponent>(com);
	for (USceneComponent* c : com)
	{
		if (c->GetName() == "ParkourArrowGroup")
		{
			arrow = c;
			break;
		}
...
void UCParkourComponent::LineTrace(EParkourArrowType InType)
{
	...
	UKismetSystemLibrary::LineTraceSingle(GetWorld(), start, end, ETraceTypeQuery::TraceTypeQuery2, false, ignores, DebugType, HitResults[(int32)InType], true, color, FLinearColor::White);
...
bool UCParkourComponent::Check_Obstacle()
{
	...
	if(center.Equals(left) == false)
		return false;
	if(center.Equals(right) == false)
		return false;
	...
	if(yaw <= AvailableFrontAngle ==false)
		return false;
...
void UCParkourComponent::End_DoParkour()
{
	...
	UCMovementComponent* movement = Cast<UCMovementComponent>(OwnerCharacter->GetComponentByClass(UCMovementComponent::StaticClass()));
...
{
	if(HitResults[(int32)EParkourArrowType::Ceil].bBlockingHit == false)
		return false;

	const TArray<FParkourData>* datas = DataMap.Find(EParkourType::Climb);
	if((*datas)[0].MinDistance < HitDistance == false) 
		return false;
	if((*datas)[0].MaxDistance > HitDistance == false) 
		return false;
	if(FMath::IsNearlyEqual((*datas)[0].Extent, HitObstacleExtent.Z, 10) == false)
		return false;
...
bool UCParkourComponent::Check_FallMode()
{
	...
	const TArray<FParkourData>* datas = DataMap.Find(EParkourType::Fall);
	if((*datas)[0].MinDistance < distance == false)
		return false;
	if((*datas)[0].MaxDistance > distance == false)
		return false;
...
{
	if(HitResults[(int32)EParkourArrowType::Floor].bBlockingHit == true)
		return false;
...
{
	if(HitResults[(int32)EParkourArrowType::Ceil].bBlockingHit == true)
		return false;
